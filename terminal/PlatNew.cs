...
namespace HaiFeng
{
    public partial class PlatNew : UserControl
    {
        ...
        public PlatNew(string server_addr = "broadcast.eicp.net:55881")
        ...
        void SaveStrategy()
        {
            ...
            foreach(Strategy stra in this._dicStrategies.Values)
            {
                ...
                foreach(Data dataold in stra.Datas)
                {
                    ...
                    {
                        Interval = dataold.Interval,
                        IntervalType = dataold.IntervalType,
                        Instrument = dataold.Instrument,
                        InstrumentOrder = dataold.InstrumentOrder,
                    };
                    data.InstrumentInfo = new InstrumentInfo
                    {
                        InstrumentID = dataold.InstrumentInfo.InstrumentID,
                        ProductID = dataold.InstrumentInfo.ProductID,
                        PriceTick = dataold.InstrumentInfo.PriceTick,
                        VolumeMultiple = dataold.InstrumentInfo.VolumeMultiple,
                    };
                    strategy.Datas.Add(data);

                }
                list.Add(strategy);
            }
            list.Sort((e1, e2) => e1.Name.CompareTo(e2.Name));
            File.WriteAllText("strategies.cfg", JsonConvert.SerializeObject(list, new JsonSerializerSettings{TypeNameHandling = TypeNameHandling.Auto }));
        }
        ...
        void LoadStrategy()
        {
            if (File.Exists("strategies.cfg"))
            {
                ...
                foreach (StrategyConfigNew stra in list)
                {
                    ...
                    foreach (Type t in this.ComboBoxType.Items)
                    {
                        if (t.FullName == stra.TypeFullName)
                        {
                            straType = t;
                            break;
                        }
        ...
        private String toIntervalString(int interval, EnumIntervalType intervalType)
        {
            switch(intervalType)
            {
                ...
                default:
                    return Enum.GetName(typeof(EnumIntervalType), intervalType);
            }
        }
        ...
        void ReadServerConfig()
        {
            ...
            foreach (var file in Directory.GetFiles("./brokerlist/", "*.xml", SearchOption.TopDirectoryOnly))
            {
                doc.Load(file);
