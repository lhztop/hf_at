...
namespace HaiFeng
{
    ...
    public abstract partial class StrategyBase : CustomTypeDescriptor
    {
		...
		public delegate void RtnOrder(OrderItem pOrderItem, Data pData, StrategyBase pStrategy);
    ...
    /// </summary>
    [DefaultProperty("Name")]
	public abstract partial class Strategy : StrategyBase
    {

        /// <summary>
        /// </summary>
        protected Strategy() : base()
		{
		}
		...
		public virtual void Buy(double pLots, double pPrice, string pRemark = "")
		{
		...
		/// <param name="pPrice"> 价格  (非PriceTick整数倍会报错,请先行处理.)</param>
		/// <param name="pRemark">注释</param>
		public virtual void Sell(double pLots, double pPrice, string pRemark = "")
		{
		...
		/// <param name="pPrice"> 价格  (非PriceTick整数倍会报错,请先行处理.)</param>
		/// <param name="pRemark">注释</param>
		public virtual void SellShort(double pLots, double pPrice, string pRemark = "")
		{
		...
		/// <param name="pPrice"> 价格  (非PriceTick整数倍会报错,请先行处理.)</param>
		/// <param name="pRemark">注释</param>
		public virtual void BuyToCover(double pLots, double pPrice, string pRemark = "")
		{
    ...
    public abstract class MultiInstrumentStrategy : StrategyBase
    {
        protected Dictionary<string, List<Data>> instrumentMap = new Dictionary<string, List<Data>>();
        public Dictionary<string, List<Data>> InstrumentMap { get => this.instrumentMap; }

        protected MultiInstrumentStrategy(): base()
        {
        }
        ...
        public override void Initialize()
        {
            ...
            foreach(Data data in this.Datas)
            {
                if (data.Instrument != data.InstrumentOrder)
                {
                    throw new ArgumentException(string.Format("need instrument and instrumentorder equal, instrument: {0}, instrumentorder: {1}", data.Instrument, data.InstrumentOrder));
                }
        ...
        public virtual void Sell(string instrument, double pLots, double pPrice, string pRemark = "")
        {
            ...
            {
                this.instrumentMap[instrument][0].Sell((int)pLots, pPrice, pRemark);
            } else
            {
                throw new KeyNotFoundException(instrument + " not in strategy instrument");
            }
        }
        ...
        public virtual void SellShort(string instrument, double pLots, double pPrice, string pRemark = "")
        {
            if (this.instrumentMap.ContainsKey(instrument))
            {
                this.instrumentMap[instrument][0].SellShort((int)pLots, pPrice, pRemark);
            }
            else
            {
                throw new KeyNotFoundException(instrument + " not in strategy instrument");
        ...
        public virtual void Buy(string instrument, double pLots, double pPrice, string pRemark = "")
        {
            if (this.instrumentMap.ContainsKey(instrument))
            {
                this.instrumentMap[instrument][0].Buy((int)pLots, pPrice, pRemark);
            }
            else
            {
                throw new KeyNotFoundException(instrument + " not in strategy instrument");
        ...
        public virtual void BuyToCover(string instrument, double pLots, double pPrice, string pRemark = "")
        {
            if (this.instrumentMap.ContainsKey(instrument))
            {
                this.instrumentMap[instrument][0].BuyToCover((int)pLots, pPrice, pRemark);
            }
            else
            {
                throw new KeyNotFoundException(instrument + " not in strategy instrument");
